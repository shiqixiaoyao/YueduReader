            ...
            List {
                Section {
                    Button { showingFileImporter = true } label: {
                        Label("从本地文件导入", systemImage: "doc.badge.plus")
                    }
                    Button { showingAddSource = true } label: {
                        Label("手动添加书源", systemImage: "plus.circle")
                    }
                } header: {
                    Text("本地导入")
                } footer: {
                    Text("选择书源 JSON 文件，可一次导入多个书源。")
                }
    ...
    private func importSources(from result: Result<[URL], Error>) {
        do {
            ...
            for url in urls {
                ...
                for item in try BookSourceFileParser.parse(data: data) {
...
private enum BookSourceFileParser {
    static func parse(data: Data) throws -> [BookSource] {
        ...
        return dictionaries.compactMap { item in
            ...
            return BookSource(name: name, baseURL: baseURL, searchURL: searchURL, enabled: enabled)
        }
    }
